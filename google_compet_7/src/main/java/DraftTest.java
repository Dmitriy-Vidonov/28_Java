    ...
    public static void main(String[] args) {
      ...
      /*  int i, karetka;
       ...
       /* for(int i=0; i<wordsArray.length-2; i++) { //основной цикл по словам. число итераций = числу слов в строке
        ...
        int dlina = 10;
        String s = "аллеунде цум гебурстаген шойцен";
        String subs = "цум";
        ...
        while (i1 < s.length()) {
            if(s.length() <= dlina) {
                spisok_strok.add(s); //в список строк добавляем всю строку, если она меньше нужной ширины
                s = "";
            } else {
                count = 0;
                for(int i=dlina; i>0; i--) {
                    if(s.charAt(i) == ' ') {
                        count++; //считаем число пробелов в рамках ширины
                    }
                }
                if(count > 0) {
                    for(int i=dlina; i>0; i--) {
                        if(s.charAt(i) == ' ') { //проходим строку как бы с конца, накладывая "маску" ширины. ищем ' '
        ...
        System.out.println(Arrays.asList(spisok_strok)); //возврат символа из строки по индексу (индекс с нуля)
